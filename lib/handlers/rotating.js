...
function RotatingFileHandler(options) {
    ...
    if (this._rule == 'time' && 'timeRate' in options) {
    ...
    if (this._rule == 'time'){
        this.fileNameFormat = compileFormat(this._oldFile, rotates[this._timeRate].format || '%Y%m%d%H%M%S');
    }
...
RotatingFileHandler.prototype.emit = function emit(record, callback) {
    ...
    new Promise((resolve, reject) => {
        if (! that._nextRotateCheckTime || that._nextRotateCheckTime - Date.now() < 0) {
            that._nextRotateCheckTime = that.isRuleSize() ? new Date(Date.now() + maxSizeStatFileInterval) : that._nextRotate(new Date());
...
FileHandler.prototype.isRuleSize = function () {
    return this._rule == 'size';
};

FileHandler.prototype.isRuleTime = function () {
...
RotatingFileHandler.prototype.shouldRotate = function () {
    ...
    return this._getData().then(function (t) {
        ...
        if (that.isRuleTime()){
            ...
            debug(`that._birthtime: ${that._birthtime}, that._rotateStartPeriod: ${that._rotateStartPeriod.toISOString()}, _rotateAt: ${that._rotateAt.toISOString()}, nowTime: ${new Date(nowTime).toISOString()}, ${that._rotateAt.getTime() - nowTime}`);
            if (parseInt(that._rotateAt.getTime() / 1000) <= parseInt(nowTime / 1000)){
                ...
                return true;
            } else if (that._birthtime.getTime() < that._rotateStartPeriod.getTime()){
                debug(`Old file ${that._file}, need rotate, that._birthtime: ${that._birthtime}, that._rotateStartPeriod: ${that._rotateStartPeriod.toISOString()}`);
                return true;
            }
        } else {
...
RotatingFileHandler.prototype._getData = function () {
    let that = this;
    let size;
    if (this.isRuleSize() || this.isRuleTime()) {
...
RotatingFileHandler.prototype.rotate = function _rotate() {
    ...
    return new Promise(function (resolve, reject) {
        ...
        return stat(that._file).then(function (stat) {
            if (that.isRuleTime() && stat.birthtime.getTime() - that._rotateAt.getTime() >= 0 ||
                that.isRuleSize() && stat.size < that._maxSize) {
                debug(`stat.birthtime: ${stat.birthtime}, that._rotateAt: ${that._rotateAt.toISOString()}}`);
            ...
            that.isRuleTime() && debug(' -- rotating, birthtime:', stat.birthtime, ', rotateAt: ', that._rotateAt);
            that._isEnded = true;

            return (new Promise(function (resolve, reject) {
                that._stream.end(resolve);
            })).then(new Promise(function (resolve, reject) {
                resolve(that.isRuleTime() ? that._renameByTime() : that._renameBySize());
            })).then(function () {
                that._stream = that._open();
                if (that.isRuleTime()) {
                    that._rotateAt = that._nextRotate(new Date());
